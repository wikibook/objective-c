...
{
    // 엘리먼트 위치를 파악하기 위한 스택에 엘리먼트명을 추가 
...
{
    // 엘리먼트 위치를 파악하기 위한 스택에서 엘리먼트명을 삭제 
    [_elementStack removeLastObject];
    
    // 종료한 엘리먼트의 이름이 'item'으로 현재 분석중인 엘리먼트가 
    // 있는 경우에는 배열 '_entries'에 추가해서 분석함 
...
{
    // 엘리먼트 위치에 따라 텍스트노드의 의미가 다름 
    // 위치의 판정을 쉽게 하기 위해 파일 패스와 같이
    // 문자열로 함 
